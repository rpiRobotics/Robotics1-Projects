...
%         drawnow
        ...
        eef_pos = robot.endeffector_positions;
        Data_rotated = RR(:,:,end)*[zeros(1280,1),-X,-Z]';
        Storage(len*(ct_stor-1)+1:len*ct_stor,1) = Data_rotated(1,:)'+1000*eef_pos(1,end);
        Storage(len*(ct_stor-1)+1:len*ct_stor,2) = Data_rotated(2,:)'+1000*eef_pos(2,end);
        Storage(len*(ct_stor-1)+1:len*ct_stor,3) = Data_rotated(3,:)'+1000*eef_pos(3,end);   
        Storage(len*(ct_stor-1)+1:len*ct_stor,4) = I_scaled;
        
%         Storage(len*(ct_stor-1)+1:len*ct_stor,1) = X+1000*eef_pos(1,end);
%         Storage(len*(ct_stor-1)+1:len*ct_stor,2) = 1000*ones(len,1)*eef_pos(2,end);
%         Storage(len*(ct_stor-1)+1:len*ct_stor,3) = -Z+1000*eef_pos(3,end);   
%         Storage(len*(ct_stor-1)+1:len*ct_stor,4) = I_scaled;

%         
%         hold on
%         plot3(1000*ones(len,1)*pp(1,end),X+1000*pp(2,end),-Z+1000*pp(3,end));
        plot3(Storage(:,1),Storage(:,2),Storage(:,3),'.b','MarkerSize',0.05);
%         hold on
...
%         axis([-35,35,55,130]);
        ...
        if params.keyboard.rec_flag == 0
            ...
            J = getJacobian_task_sawyer(params.controls.q, params.def.type, params.def.H, params.def.P+[zeros(3,params.def.n),R_EE*scan_dist], params.def.n,R_EE);
        end
...
figure
scaleImax=max(Storage(:,4));
scatter3(Storage(:,1),Storage(:,2),Storage(:,3),ones(size(Storage,1),1)*0.1,[Storage(:,4)/scaleImax,Storage(:,4)/scaleImax,Storage(:,4)/scaleImax],'filled');

save('weldbead.txt','Storage','-ascii')
