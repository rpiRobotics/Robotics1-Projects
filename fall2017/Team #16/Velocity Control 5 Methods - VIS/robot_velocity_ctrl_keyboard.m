...
r_scale = 0.5;
[my_robot,my_robot_structure]=defineRobot(type,H,P,n,r_scale);
h_my_robot = createCombinedRobot(my_robot,my_robot_structure);
% axis equal; 
axis(axes_lim); view(view_port) 
%velocities
w_t = [0,0,0]';
v_t = [0,0,0]';
...
set(gcf,'keypressfcn',{@func_keypress,params});
% h_my_robot = showRobot(q,h_my_robot,axes_lim,view_port);
...
while ~params.controls.stop    
    ...
    if (counter~=0)
        ...
        %update robot
        h_my_robot = showRobot(params.controls.q,h_my_robot,axes_lim,view_port);
        [pp,RR]=fwdKin_alljoints(params.controls.q,type,H,P,n);
