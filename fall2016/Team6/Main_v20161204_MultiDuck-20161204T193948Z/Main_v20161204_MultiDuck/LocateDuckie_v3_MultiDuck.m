function [XCenter, XRadii, OriDuck] = LocateDuckie_v3_MultiDuck(img,D,N_Duck)
%%% 11/07/2016
%%% INPUT=================
%%% img:         Input image
%%% D:             Resolution decimation
%%% N_Duck:  Numbers of duckies to be located
%%% OUTPUT=================
...
b =  -2.4496;
XColor(find(XRadii<5),3) = 255;
Color_classify = (XColor*sol+b);

[Xmax,Xid]=max(Color_classify);
for i = 2:N_Duck
    [Xmax,Xidtmp]=max(Color_classify(Color_classify<Xmax));
    Xid = [Xid, Xidtmp];
end
...
Xsearch_start = Xsearch_start.*(Xsearch_start>0);
Xsearch_start(find(Xsearch_start==0))=1;
Xsearch_stop(:,1) = Xsearch_stop(:,1).*(Xsearch_stop(:,1)<=n);
Xsearch_stop(find(Xsearch_stop(:,1)==0),1)=n;
Xsearch_stop(:,2) = Xsearch_stop(:,2).*(Xsearch_stop(:,2)<=m);
Xsearch_stop(find(Xsearch_stop(:,2)==0),2)=m;

XRadii = [];
for i = 1:N_Duck
    Xsub = img(Xsearch_start(i,2):Xsearch_stop(i,2), Xsearch_start(i,1):Xsearch_stop(i,1),3);
    XRadii(i) = sqrt(length(find(Xsub==0)));
    
    %%% Duckie Orientation
    edgeIm = sobel(double(double(Xsub)<100), 0.3);
    [Edge_x,Edge_y] = find(edgeIm(:,:)==255);
    tmp =  pca([Edge_x,Edge_y]');
    [tm, tn] = size(tmp);
    if(tm<2 || tn<2)
      OriDuck(i,:,:) = ones(2,2);
    else
